...
% 提取水平边缘
gb = edge(f, 'Sobel', [], 'horizontal');  % 使用Prewitt算子进行水平边缘检测

% 提取垂直边缘
gv = edge(f, 'Sobel', [], 'vertical');  % 使用Prewitt算子进行垂直边缘检测
% 显示水平和垂直边缘
figure, imshow(gv,[]), title('Sobel算子提取垂直边缘');
figure, imshow(gb,[]), title('Sobel算子提取水平边缘');
