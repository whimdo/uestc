...
float wave(int i, float x, float y, float time) {
	...
	return 2*amplitude[i] * pow((sin(theta * frequency + time * phase)+1)/2,k);
}
...
float dWavedx(int i, float x, float y, float time) {
	...
	return k*A * cos(theta * frequency + time * phase)*pow((sin(theta * frequency + time * phase) + 1) / 2, k-1);
}
...
void keyboardCallback(int key, int x, int y) {
	...
	case GLUT_KEY_LEFT: // 左方向键
		k = std::max(k - 1, 1); // 将k减小1,但不能小于最小值1
		break;
	case GLUT_KEY_RIGHT: // 右方向键
		k = std::min(k + 1, 5); // 将k增大1,但不能大于最大值5
		break;
	default:
		break;
	}
	// 在这里可以添加其他逻辑,如重新绘制场景等
	glutPostRedisplay(); // 请求重新绘制场景
}
